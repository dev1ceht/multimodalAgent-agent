...
|---|---|---|
| `MultimodalAgentUnavailable` | critical | Check application startup, database/Redis/Qdrant reachability, then recent deploy logs. |
...
| `RagRetrievalFailures` | critical | Check Qdrant and embedding availability; do not silently enable a fallback for required retrieval. |
