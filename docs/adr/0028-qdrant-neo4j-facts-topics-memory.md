...
Facts 另外维护按用户隔离的内存 BM25 索引，用于精确术语、名称、时间表达和缩写召回。MySQL 仍是规范源：索引在首次查询时按用户懒重建，并在异步 Fact 投影成功后做幂等增量更新；进程重启不需要迁移额外持久化状态。中文文本使用字符 unigram/bigram，拉丁字母与数字按归一化词项索引。

长期记忆召回按以下顺序融合：

1. Qdrant 召回 Fact 向量候选，BM25 召回 Fact 关键词候选，两路使用可配置 RRF 或归一化加权融合；
...
- 支持跨轮事实跟踪、多跳问答和因果链推理。
- BM25 补足稠密向量对精确词项的漏召回；索引是进程内可重建投影，会增加与活跃用户 Facts 规模相关的堆内存占用。
