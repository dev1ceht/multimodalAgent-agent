...
Accepted; bootstrap detail superseded by ADR 0019
...
The application previously relied on Hibernate `ddl-auto=update`. That is useful for local development but does not provide an auditable production schema history. Existing MySQL deployments already have tables created by the application, so the first migration rollout must be incremental and must preserve the version-1 baseline.
...
- The migration path is auditable for existing MySQL installations and now has a complete from-scratch bootstrap through `V0`; previously recorded `V1` checksums remain stable.
