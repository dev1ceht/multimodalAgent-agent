...
try {
    ...
    $sectionUniqueColumns = & mysql --protocol=TCP --host=127.0.0.1 --port=$HostPort `
        --user=$smokeDbUser --database=$smokeDatabase --batch --skip-column-names `
        -e "SELECT GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') FROM information_schema.statistics WHERE table_schema = DATABASE() AND table_name = 'knowledge_version_sections' AND index_name = 'uk_kv_section_attempt_parent_key';"
    if ($LASTEXITCODE -ne 0 -or ($sectionUniqueColumns | Select-Object -First 1).Trim() -ne "knowledge_version_id,build_attempt_scope,parent_key") {
