...
try {
    ...
    # Verifies Flyway V0, V1, V2, V3, V4, V5 in order on a fresh database.
    $expectedVersions = @("0", "1", "2", "3", "4", "5")
    ...
    Write-Host "MySQL migration smoke passed: Flyway V0 through V5 and ddl-auto=validate startup succeeded."
} catch {
    $scriptFailed = $true
    $failureRecord = $_
    $diagnosticLines = @(
