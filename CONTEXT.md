...
**基准模型标签**:
为候选模型创建的中性Ollama运行身份，包含各自正确聊天模板、统一推理参数和逐字一致的模型无关系统提示。系统提示只定义共同的校园心理关怀角色、安全边界和任务服从规则，不写入模型身份。基准模型标签与日常运行标签分离。
_Avoid_: 生产模型标签、原始GGUF文件、缺失或内容不一致的系统提示、带模型专属身份的系统提示
