...
**当前检索实现评测**:
针对应用启动时实际生效的检索模式、知识版本和 Top-K 运行完整 RAG 链路。评测器从应用状态读取并记录配置，不切换检索器，也不要求固定为 Elasticsearch；结果只代表该次记录下来的当前实现。
_Avoid_: 评测脚本覆盖应用配置、未记录配置的结果、把不同配置结果直接视为同一基线

**本地检索模式**:
应用当前配置选择本地轻量文本检索时得到的实际运行模式。它可以直接接受当前实现评测，但报告必须记录 `LOCAL_BASELINE`，不能与其他检索模式的结果混为同一配置。
_Avoid_: Elasticsearch 结果、未标注检索模式的汇总

**当前向量配置**:
应用当前运行配置中实际使用的向量服务、模型和维度。使用向量检索时，查询和知识文档必须由兼容配置生成向量；评测脚本不再强制指定供应商、模型或维度。
_Avoid_: 评测脚本覆盖向量配置、查询与知识向量不兼容、未记录的向量配置
...
**知识父块**:
知识文档中的完整语义章节，是命中子块后提供给回答模型的上下文单元。父块按标题或段落组织，并受证据字符预算约束。
_Avoid_: Elasticsearch join 文档、任意相邻片段、整篇文档

**知识子块**:
携带完整标题路径、父块标识和精确偏移量的最小检索单元。Embedding、BM25 和重排作用于子块；最终回答上下文按父块去重回填。
_Avoid_: 最终展示证据、跨父块重叠、无父级位置的字符串
...
**基准模型标签**:
为可选模型对比保留的中性 Ollama 运行身份，包含各自正确聊天模板、统一推理参数和逐字一致的模型无关系统提示。当前实现评测不要求使用这些标签，实际模型名称从应用状态记录。
_Avoid_: 把结果目录标签当作模型选择器、评测脚本强制切换模型、未记录实际模型

**主性能结果**:
当前评测设备上运行完整 RAG 链路得到的用户体验指标，必须与该次记录的应用配置一起解释。
...
**运行清单**:
描述一次评测所使用模型权重、代码、知识库、切块、向量服务、检索配置、裁判快照、硬件和随机设置的不可变记录。缺少完整运行清单的结果不能进入正式比较。只用于查看当前效果的评测可以保存非敏感应用配置快照，但必须显式标记为不具备正式比较资格。
