...
**当前检索实现评测**:
针对应用启动时实际生效的检索模式、知识版本和 Top-K 运行完整 RAG 链路。评测器从应用状态读取并记录配置，不切换检索器，也不要求固定为 Qdrant；结果只代表该次记录下来的当前实现。
...
_Avoid_: Qdrant join 文档、任意相邻片段、整篇文档

**知识子块**:
携带完整标题路径、父块标识和精确偏移量的最小检索单元。Embedding 向量召回和重排作用于子块；最终回答上下文按父块去重回填。
